//! player_movement.rs
//! Smooth “pseudo-physics” driving + working **game-pad** controls.
...
const MAX_STEER_DEG:        f32 = 85.0;
const MIN_STEER_DEG:        f32 = 30.0;
...
pub fn player_movement_system(engine: &mut Engine) -> Result<()> {
    ...
    // Throttle / brake (triggers win, else left-stick Y, else keyboard)
    let mut throt = 0.0;
    let mut brake = 0.0;
    ...
    if trig_rt > DZ || trig_lt > DZ {
        ...
        if stick_y > 0.0 { throt = stick_y; }         // stick up
        else             { brake = -stick_y; }        // stick down
    }
