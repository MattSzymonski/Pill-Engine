...
//!
//! Used by both `game.rs` (normal play mode) and `benchmark.rs` (benchmark mode).
...
pub const ORIGIN: (f32, f32, f32) = (0.0, 0.0, 0.0);
...
///
/// The caller provides an RNG (e.g. `thread_rng()` for game mode or a seeded
        ...
        if citizen.path_points.is_empty() {
            for _ in 0..PATH_POINT_COUNT {
                let point = Vector3f::new(
                    ORIGIN.0 + rng.gen_range(-PATH_WANDER_RADIUS_XZ..PATH_WANDER_RADIUS_XZ),
                    ORIGIN.1,
                    ORIGIN.2 + rng.gen_range(-PATH_WANDER_RADIUS_XZ..PATH_WANDER_RADIUS_XZ),
                );
