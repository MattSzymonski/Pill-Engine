...
// All uniform buffers use the same bind group layout, always registered as index PARAMETERS_UNIFORM_BIND_GROUP_LAYOUT_INDEX (0, what means that they are bound to set = 0)
// Engine parameters are sent as uniform buffer bound to binding = 0
// Camera parameters are sent as uniform buffer bound to binding = 1
// Material parameters are sent as uniform buffer bound to binding = 2
// All textures use the same bind group layout, always registered as index TEXTURES_BIND_GROUP_LAYOUT_INDEX (1, what means that they are bound to set = 1)
...
layout(set = 0, binding = 2) uniform material_parameters {
    vec3 tint;
    float specularity;
};
