...
// This is just typemap_rev with modified names (Value->Storage, TypeMap->ResourceMap, TypeMapKey->Resource)
...
/// Resource is used to declare key types that are eligible for use
/// with [`ResourceMap`].
///
/// [`ResourceMap`]: struct.ResourceMap.html
pub trait Resource: Any {
    /// Defines the value type that corresponds to this `Resource`.
    type Storage: Send + Sync;
}

/// ResourceMap is a simple abstraction around the standard library's [`HashMap`]
/// type, where types are its keys. This allows for statically-checked value
/// retrieval.
///
/// [`HashMap`]: std::collections::HashMap
pub struct ResourceMap(HashMap<TypeId, Box<(dyn Any + Send + Sync)>>);
...
impl ResourceMap {
    ...
    /// Returns `true` if the map contains a value for the specified [`Resource`].
    ///
    /// ```rust
    /// use typemap_rev::{ResourceMap, Resource};
    ///
    /// struct Number;
    ///
    ...
    /// Inserts a new value based on its [`Resource`].
    ...
    /// Retrieve the entry based on its [`Resource`]
    ///
    /// [`Resource`]: trait.Resource.html
    #[inline]
    ...
    /// Retrieve a reference to a value based on its [`Resource`].
    ...
    /// Retrieve a mutable reference to a value based on its [`Resource`].
    /// Returns `None` if it couldn't be found.
    ///
    /// ```rust
    /// use typemap_rev::{ResourceMap, Resource};
    ...
    /// Removes a value from the map based on its [`Resource`], returning the value or `None` if
    ...
    ///
    /// let mut map = ResourceMap::new();
    /// map.insert::<Text>(String::from("Hello ResourceMap!"));
...
/// A view into a single entry in the [`ResourceMap`],
