...
use std::{
    collections::HashMap, env, fs::{remove_file, rename, File}, io::{ BufRead, BufReader, Write }, path::PathBuf, sync::Arc, time::{Duration, Instant}
};
...
struct FileWatchers {
    ...
    game_project_resource_files_watcher: FileWatcher,
}

struct ProjectPaths {
    current_path: PathBuf,
    data_folder_path: PathBuf,
    resources_folder_path: PathBuf,
    config_path: PathBuf,
    game_dynamic_library_path: PathBuf,
    game_dynamic_library_hot_reloaded_path: PathBuf,
}

fn create_window(config: &Config, resource_folder_path: PathBuf) -> WindowData {
...
fn check_and_reload_game(
    ...
    if let Some(paths) = file_watchers.game_project_resource_files_watcher.get_changes() {
    ...
    if let Some(_) = file_watchers.game_dynamic_library_files_watcher.get_changes() {
        ...
        let mut new_engine = Engine::new(game, renderer, config.clone());
...
fn create_file_watchers(data_folder_path: &PathBuf, resources_folder_path: &PathBuf) -> FileWatchers {
    let game_dynamic_library_files_watcher = FileWatcher::new(data_folder_path.clone());
    let game_project_source_files_watcher = FileWatcher::new(resources_folder_path.join("src"));
    let game_project_resource_files_watcher = FileWatcher::new(resources_folder_path.clone());

    FileWatchers {
        game_dynamic_library_files_watcher,
        game_project_source_files_watcher,
        game_project_resource_files_watcher,
    }
}
...
fn main_loop(
    ...
    let mut file_watchers: Option<FileWatchers> = if development_mode {
        Some(create_file_watchers(&project_paths.data_folder_path, &project_paths.resources_folder_path))
    } else {
        None
    };
...
fn main() {
    ...
    let data_folder_path = current_path.join("data");
    let resources_folder_path = if development_mode {
        data_folder_path.join("res") // <EXE_LOCATION>/data/res
    } else {
        current_path.parent().unwrap().parent().unwrap().join("res") // <GAME_PROJECT_ROOT>/res
    };
    let config_path = resources_folder_path.join("config.ini");
    let game_dynamic_library_path = data_folder_path.join("pill_game.dll");
    let game_dynamic_library_hot_reloaded_path = data_folder_path.join("pill_game_hot_reloaded.dll");
    ...
    // Create windows
    let window_data = create_window(&config, project_paths.resources_folder_path.clone());
    ...
    let mut engine: Option<Engine> = Some(Engine::new(game, renderer, config.clone()));
