//! Image-based lighting bake — the single source of the IBL math.
//!
//! Pure CPU, no filesystem: usable both at build time (the `EquirectToIBL` rule) and at runtime
//! (games baking from a fetched/embedded HDR, including on WASM under the `runtime` feature).
//! Inputs/outputs are plain f32 RGBA buffers; callers handle I/O and GPU upload.

use std::f32::consts::PI;

/// Decodes a Radiance `.hdr` equirectangular panorama and resizes it to 512×256 RGBA f32.
/// (Runtime equivalent of the build-time HDR decode step.) Bilinear downsample is plenty for a
/// low-frequency environment map.
pub fn equirect_from_hdr(bytes: &[u8]) -> anyhow::Result<(Vec<f32>, u32, u32)> {
    let mut decoder = zune_hdr::HdrDecoder::new(bytes);
    let rgb: Vec<f32> = decoder
        .decode()
        .map_err(|e| anyhow::anyhow!("decode HDR: {e:?}"))?;
    let (src_w, src_h) = decoder
        .get_dimensions()
        .ok_or_else(|| anyhow::anyhow!("HDR has no dimensions"))?;

    let (out_w, out_h) = (512usize, 256usize);
    let mut pixels = Vec::with_capacity(out_w * out_h * 4);
    let sample = |x: usize, y: usize, c: usize| rgb[(y * src_w + x) * 3 + c];
    for oy in 0..out_h {
        let fy = ((oy as f32 + 0.5) / out_h as f32 * src_h as f32 - 0.5).max(0.0);
        let y0 = (fy as usize).min(src_h - 1);
        let y1 = (y0 + 1).min(src_h - 1);
        let wy = fy - y0 as f32;
        for ox in 0..out_w {
            let fx = ((ox as f32 + 0.5) / out_w as f32 * src_w as f32 - 0.5).max(0.0);
            let x0 = (fx as usize).min(src_w - 1);
            let x1 = (x0 + 1).min(src_w - 1);
            let wx = fx - x0 as f32;
            let mut rgba = [0.0f32, 0.0, 0.0, 1.0];
            for (c, out) in rgba.iter_mut().take(3).enumerate() {
                let top = sample(x0, y0, c) + (sample(x1, y0, c) - sample(x0, y0, c)) * wx;
                let bot = sample(x0, y1, c) + (sample(x1, y1, c) - sample(x0, y1, c)) * wx;
                *out = top + (bot - top) * wy;
            }
            pixels.extend_from_slice(&rgba);
        }
    }
    Ok((pixels, out_w as u32, out_h as u32))
}
...
pub fn bake_all(equirect: &[f32], width: u32, height: u32) -> (Vec<f32>, Vec<Vec<f32>>, Vec<f32>) {
    ...
    let specular_mips = bake_specular_mips(&eq);
...
/// Specular prefilter mip chain (base 128×64, 5 levels), Rgba32Float HDR. Mip i uses roughness
/// i/4 to match the shader's `roughness * MAX_REFLECTION_LOD(4)`.
pub fn bake_specular_mips(eq: &Equirect) -> Vec<Vec<f32>> {
    const MIP_ROUGHNESS: [f32; 5] = [0.04, 0.25, 0.5, 0.75, 1.0];
    let mut mips: Vec<Vec<f32>> = Vec::with_capacity(MIP_ROUGHNESS.len());
    for (mip, &r) in MIP_ROUGHNESS.iter().enumerate() {
        let w = (128u32 >> mip).max(1);
        let h = (64u32 >> mip).max(1);
        mips.push(compute_specular_prefilter(eq, w, h, r));
    }
    mips
}
...
/// Equirect panorama as flat RGBA f32 (linear). Build-time callers that read from an RTEX file
/// construct this directly; runtime callers get it from `equirect_from_hdr` / `bake_all` input.
pub struct Equirect {
    pub pixels: Vec<f32>,
    pub w: u32,
    pub h: u32,
...
/// Diffuse irradiance map (sRGB-encoded RGBA8). Public so the build rule can write it as RTEX v1.
pub fn compute_irradiance(eq: &Equirect, out_w: u32, out_h: u32) -> Vec<u8> {
    const N_PHI: u32 = 64;
...
/// One specular-prefilter mip (Rgba32Float). Public so the build rule can assemble the mip chain.
pub fn compute_specular_prefilter(
    eq: &Equirect,
    out_w: u32,
    out_h: u32,
    roughness: f32,
...
/// Environment-independent split-sum BRDF LUT (linear RGBA8). Public for the build rule.
pub fn compute_brdf_lut(w: u32, h: u32) -> Vec<u8> {
    const N: u32 = 1024;
...
// --- Color utilities (shared with studio_equirect) ---
