...
fn sync_transforms_to_physics(engine: &mut Engine, scene_handle: SceneHandle) -> Result<()> {
    ...
    for entity_handle in entities_with_physics {
        ...
        let rb_handle = {
            let rigid_body_comp = engine
                .scene_manager
                .get_entity_component::<RigidBodyComponent>(entity_handle, scene_handle)?;
...
fn sync_physics_to_transforms(engine: &mut Engine, scene_handle: SceneHandle) -> Result<()> {
    // Get entity handles with both transform and rigid body components
    let entities_with_physics: Vec<_> = {
        let scene = engine.scene_manager.get_scene(scene_handle)?;
        let mut entities = Vec::new();
