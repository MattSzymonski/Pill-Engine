...
impl Scene {
    ...
    pub fn get_four_component_iterator<A, B, C, D>(&self) -> Result<impl Iterator<Item = (EntityHandle, &A, &B, &C, &D)>>
        where
        A: Component<Storage = ComponentStorage::<A>>,
        B: Component<Storage = ComponentStorage::<B>>,
        C: Component<Storage = ComponentStorage::<C>>,
    ...
    {
        // Generate filter bitmask
        let filter_bitmask = self.get_component_bitmask::<A>()? | self.get_component_bitmask::<B>()? | self.get_component_bitmask::<C>()? | self.get_component_bitmask::<D>()?;
        ...
        let storage_b: &Vec<Option<B>> = self.components.get::<B>().unwrap().data.as_ref();
        let storage_c: &Vec<Option<C>> = self.components.get::<C>().unwrap().data.as_ref();
        let storage_d: &Vec<Option<D>> = self.components.get::<D>().unwrap().data.as_ref();
        ...
        let iterator = entities.iter()
            ...
            {(
                ...
                storage_b.get(h.0.index as usize).unwrap().as_ref().unwrap(),
                storage_c.get(h.0.index as usize).unwrap().as_ref().unwrap(),
                storage_d.get(h.0.index as usize).unwrap().as_ref().unwrap(),
            )});

        Ok(iterator)
    }

    pub fn get_four_component_iterator_mut<A, B, C, D>(&mut self) -> Result<impl Iterator<Item = (EntityHandle, &mut A, &mut B, &mut C, &mut D)>>
        where
        A: Component<Storage = ComponentStorage::<A>>,
        B: Component<Storage = ComponentStorage::<B>>,
        C: Component<Storage = ComponentStorage::<C>>,
        D: Component<Storage = ComponentStorage::<D>>,
    {
        ...
        let iterator = entities.iter()
            ...
            {(
                ...
                Self::unsafe_mut_cast(storage_b).get_mut(h.0.index as usize).unwrap().as_mut().unwrap(),
                Self::unsafe_mut_cast(storage_c).get_mut(h.0.index as usize).unwrap().as_mut().unwrap(),
                Self::unsafe_mut_cast(storage_d).get_mut(h.0.index as usize).unwrap().as_mut().unwrap(),
            )});
