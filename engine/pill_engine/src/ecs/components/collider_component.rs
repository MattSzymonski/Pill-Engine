use crate::{
    ecs::{
        components::rigid_body_component, Component, ComponentStorage, DeferredUpdateComponent,
        DeferredUpdateComponentRequest, DeferredUpdateManagerPointer, EntityHandle,
        PhysicsWorldComponent, RigidBodyComponent, SceneHandle,
    },
    engine::Engine,
};
...
impl ColliderComponent {
    ...
    fn post_deferred_update_request(&mut self, request_variant: usize) {
        if let Some(update_manager) = &mut self.deferred_update_manager {
            ...
            let request = DeferredUpdateComponentRequest::<ColliderComponent>::new(
                entity_handle,
                scene_handle,
                request_variant,
            );
    ...
    fn deferred_update(&mut self, engine: &mut Engine, request: usize) -> Result<()> {
        match request {
            DEFERRED_REQUEST_VARIANT_ADD => {
                ...
                let rigid_body_handle: Option<RigidBodyHandle> = rigid_body_component_storage
                    .data
