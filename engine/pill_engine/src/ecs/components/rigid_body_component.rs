use crate::{
    ecs::{
        Component, ComponentStorage, DeferredUpdateComponent, DeferredUpdateComponentRequest, DeferredUpdateManagerPointer, EntityHandle, PhysicsWorldComponent, SceneHandle
    }, engine::Engine
};
...
impl RigidBodyComponent {
    ...
    fn post_deferred_update_request(&mut self, request_variant: usize) {
        if self.deferred_update_manager.is_some() {
            ...
            let request = DeferredUpdateComponentRequest::<RigidBodyComponent>::new(entity_handle, scene_handle, request_variant);
