...
pub struct PostprocessingEffectDefinitionBuilder {
    postprocessing_effect_definition: PostprocessingEffectDefinition,
}

impl PostprocessingEffectDefinitionBuilder {
    pub fn new(name: &str, shader_handle: ShaderHandle) -> Self {
        Self {
            postprocessing_effect_definition: PostprocessingEffectDefinition::new(
                name,
                shader_handle,
                None,
            ),
    ...
    pub fn shader(mut self, shader_handle: ShaderHandle) -> Result<Self> {
        self.postprocessing_effect_definition.set_shader(shader_handle)?;
        Ok(self)
    }
}

pub struct PostprocessingEffectDefinition {
    pub name: String,
    pub shader_handle: ShaderHandle,
    pub(crate) material_handle: Option<MaterialHandle>,
}

impl PostprocessingEffectDefinition {
    pub fn builder(name: &str) -> PostprocessingEffectDefinitionBuilder {
        PostprocessingEffectDefinitionBuilder::new(name)
    }
...
pill_core::define_new_pill_slotmap_key! { 
    pub struct PostprocessingEffectDefinitionHandle;
}

pub struct PostprocessingEffect {
    pub postprocessing_effect_definition_handle_name: String,
    pub is_enabled: bool,
    pub opacity: f32,
    pub postprocessing_effect_definition_handle: PostprocessingEffectDefinitionHandle,
    pub parameters_store: MaterialParametersStore,
}
...
pub struct PostprocessingEffectRendererData {
    ...
    pub material_parameters: MaterialParametersStore,
    pub influence: f32,
}
...
pub fn register_color_adjustments_postprocessing_effect(engine: &mut Engine) -> Result<()> {
    ...
    let shader_handle = engine.add_default_resource(
        Shader::new(
            ...
            vec![
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_OPACITY_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_OPACITY_PARAMETER_SLOT.1)),
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_EXPOSURE_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_EXPOSURE_PARAMETER_SLOT.1)),
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_TINT_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_TINT_PARAMETER_SLOT.1)),
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_WHITE_BALANCE_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_WHITE_BALANCE_PARAMETER_SLOT.1)),
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_HUE_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_HUE_PARAMETER_SLOT.1)),
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_SATURATION_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_SATURATION_PARAMETER_SLOT.1)),
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_CONTRAST_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_CONTRAST_PARAMETER_SLOT.1)),
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_BRIGHTNESS_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_BRIGHTNESS_PARAMETER_SLOT.1)),
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_INVERT_FLAG_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_INVERT_FLAG_PARAMETER_SLOT.1)),
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_GAMMA_PARAMETER_SLOT.0.to_string(), ShaderValueParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_GAMMA_PARAMETER_SLOT.1)),
            ].into_iter().collect(),
            vec![
                (COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_SCENE_TEXTURE_SLOT.0.to_string(), ShaderTextureParameterSlot::new(COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_SCENE_TEXTURE_SLOT.1, COLOR_ADJUSTMENTS_POSTPROCESSING_SHADER_SCENE_TEXTURE_SLOT.2)),
            ].into_iter().collect(),
            true,
            true
        )
    ...
    let postprocessing_effect_definition = PostprocessingEffectDefinition {
        name: "ColorAdjustments".to_string(),
        shader_handle,
        material_handle: engine.add_default_resource(
